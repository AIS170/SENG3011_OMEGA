...
def register_user():
    ...
    try:
        ...
        if e.response['Error']['Code'] == "404":
            ...
            return jsonify({"message": f"User '{username}' registered and set as current session user."}), 201
        else:
...
def get_stocks_for_news(username):
    ...
    sts_client = boto3.client('sts')
    assumed_role = sts_client.assume_role(RoleArn=CLIENT_ROLE_ARN, RoleSessionName="AssumeRoleSession1")
...
def upload_csv_to_s3(username, company_name, df, date_str=None):
    ...
    sts_client = boto3.client('sts')
    assumed = sts_client.assume_role(RoleArn=CLIENT_ROLE_ARN, RoleSessionName="AssumeRoleSession1")
